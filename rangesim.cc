...
int main(int argc, char** argv)
{
	...
	RSConMan* CM = new RSConMan(cfgFileName.data());
...
//	G4RunManager* RM = new G4RunManager();
	...
	// User actions
	RM -> SetUserInitialization(new RSActIni(CM));
...
void PrintHelp()
{
	std::cout << "usage: rangesim [-b] [-g] [-m macrofile] [-c configfile]" << std::endl;
	std::cout << std::endl;
	std::cout << "Examples:" << std::endl;
	std::cout << "  rangesim -b -m myRun.mac -c myConfig.cfg  # Run in batch mode with macro and config." << std::endl;
	std::cout << "  rangesim -g                               # Run in graphical mode." << std::endl;
	std::cout << std::endl;
